...
import { getEntitiesByCourseId } from './assignment.reducer';
...
export const Assignment = () => {
  ...
  // Extract courseId from the path (assuming courseId is after '/course/')
  const courseId = currentPath.split('/course/')[1]?.split('/')[0];
  ...
  return (
    <div>
      <div className="px-4">
        ...
        <div className="table-responsive">
          {Array.isArray(assignmentList) && assignmentList.length > 0 ? (
            <Table responsive>
              <thead>
                <tr>
                  <th className="hand" onClick={sort('name')}>
